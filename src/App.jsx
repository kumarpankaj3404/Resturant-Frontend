...
function AppContent() {
  ...
  const handleLoginSuccess = (userData) => {
    const userInfo = userData || { name: "Guest", role: "user" };
    localStorage.setItem("user", JSON.stringify(userInfo));
    closeLoginModal();
    navigate("/profile");
  };
    ...
    <>
      {showWelcome ? (
        ...
        >
          <div className="z-10 font-bold text-center text-white bg-black h-screen flex flex-col justify-center items-center">
            <motion.h1
              className="mb-8 tracking-widest uppercase font-heading sm:text-xl md:text-5xl"
              initial={{ y: -150, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              transition={{ duration: 1 }}
            >
            ...
            <motion.h2
              className="text-3xl font-bold mb-14 sm:text-4xl md:text-6xl font-heading"
              initial={{ y: 150, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              transition={{ delay: 1, duration: 0.75 }}
            >
        ...
        <>
          {/* ✅ ONLY ROUTES (Layouts handle UI) */}
          <Routes>
            <Route
              path="/*"
              element={<AppRoutes onLoginClick={openLoginModal} />}
            />
          </Routes>
